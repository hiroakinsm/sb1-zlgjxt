<template>
  <div class="mt-8">
    <div class="bg-white shadow rounded-lg p-6">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">解析進捗状況</h3>
      
      <!-- 単一ソース解析タスク -->
      <div v-for="task in singleTasks" :key="task.id" class="mb-4">
      ...
      <!-- サマリ解析タスク -->
      <div v-if="summaryTask" class="mt-6">
        <div class="flex justify-between items-center mb-2">
...
const isAllCompleted = computed(() => {
  const allSingleTasksCompleted = props.singleTasks.every(
    task => task.status === 'completed'
  )
  return allSingleTasksCompleted && 
         (!props.summaryTask || props.summaryTask.status === 'completed')
})
